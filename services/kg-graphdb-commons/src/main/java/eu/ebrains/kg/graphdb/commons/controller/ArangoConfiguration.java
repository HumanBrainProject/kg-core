...
public class ArangoConfiguration {
    ...
    @Bean
    @Qualifier("live")
    public ArangoDatabaseProxy produceLiveDB(ArangoDB.Builder arangoDB) {
        return new ArangoDatabaseProxy(arangoDB.build(), "kg1-live");
    }

    @Bean
    @Qualifier("released")
    public ArangoDatabaseProxy produceReleasedDb(ArangoDB.Builder arangoDB) {
        return new ArangoDatabaseProxy(arangoDB.build(), "kg1-release");
    }

    @Bean
    @Qualifier("nativeMeta")
    public ArangoDatabaseProxy produceDefaultMetaDb(ArangoDB.Builder arangoDB) {
