...
public class ArangoRepositoryTypes {
    ...
    public Paginated<NormalizedJsonLd> getAllTypes(String client, DataStage stage, boolean withProperties, PaginationParam pagination) {
        ArangoDatabase db = databases.getMetaByStage(stage);
        AQLQuery typeStructureQuery = createTypeStructureQuery(db, client, null, null, null, withProperties, pagination);
        return arangoRepositoryCommons.queryDocuments(db, typeStructureQuery);
    }

    public Paginated<NormalizedJsonLd> getTypesForSpace(String client, DataStage stage, Space space, boolean withProperties,  PaginationParam pagination) {
        ArangoDatabase db = databases.getMetaByStage(stage);
        AQLQuery typeStructureQuery = createTypeStructureQuery(db, client, null, null, space,withProperties, pagination);
        return arangoRepositoryCommons.queryDocuments(db, typeStructureQuery);
    }
    ...
    private void ensureTypeStructureCollections(ArangoDatabase db){
        List<ArangoCollectionReference> collections = Arrays.asList(ArangoCollectionReference.fromSpace(InternalSpace.ADMIN),
    ...
    private AQLQuery createTypeStructureQuery(ArangoDatabase db, String client, List<Type> types, String targetTypesForProperty, Space space, boolean withProperties, PaginationParam paginationParam) {
        ...
        bindVars.put("clientName", client);
        bindVars.put("globalSpace", ArangoCollectionReference.fromSpace(InternalSpace.ADMIN).getCollectionName());
        ...
        if(targetTypesForProperty!=null && !targetTypesForProperty.isBlank()){
            ...
            aql.addLine(AQL.trust("FOR type IN @@types"));
        ...
        if(withProperties){
            ...
            aql.addLine(AQL.trust("LET docsContributingToSpace2Property = FIRST(FOR docContributingToSpace2Property, docContributingToSpace2PropertyEdge IN 1..1 INBOUND spaceType2propertyEdge @@documentRelation"));
            ...
            aql.addLine(AQL.trust("FOR g IN 1..1 INBOUND   @@metaProperty"));
            ...
            aql.addLine(AQL.trust("RETURN MERGE({\""+EBRAINSVocabulary.META_OCCURRENCES+"\": countOccurrences}, globalProperty, targetByTypeWithCount))"));
...
//
        ...
        if(withProperties) {
            aql.addLine(AQL.trust("RETURN DISTINCT MERGE(UNION([UNSET(type, propertiesToRemove)], globalTypeDef, clientSpecific, globalOccurrences, globalProperties, spaces))"));
        }
        else{
            aql.addLine(AQL.trust("RETURN DISTINCT MERGE(UNION([UNSET(type, propertiesToRemove)], globalTypeDef, clientSpecific, globalOccurrences, spaces))"));
        }
