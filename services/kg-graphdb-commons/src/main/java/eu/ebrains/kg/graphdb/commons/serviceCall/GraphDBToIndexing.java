...
@Component
public class GraphDBToIndexing {
    ...
    public List<TodoItem> getTodoItems(PersistedEvent event, DataStage stage) {
        return Arrays.asList(serviceCall.post(String.format("http://kg-indexing/internal/indexing/event/todos/%s", stage), event, authContext.getAuthTokens(), TodoItem[].class));
    }
}
