...
public class GraphDBScopesAPIRest implements GraphDBScopes {

    private final GraphDBScopesAPI graphDBScopesAPI;

    public GraphDBScopesAPIRest(GraphDBScopesAPI graphDBScopesAPI) {
        this.graphDBScopesAPI = graphDBScopesAPI;
    }
    ...
    public ScopeElement getScopeForInstance(@PathVariable("space") String space, @PathVariable("id") UUID id, @PathVariable("stage") DataStage stage, @RequestParam(value = "fetchLabels", defaultValue = "true") boolean fetchLabels){
       return this.graphDBScopesAPI.getScopeForInstance(space, id, stage, fetchLabels);
    }
