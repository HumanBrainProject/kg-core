...
package eu.ebrains.kg.ids.config;
...
@Configuration
public class IdsArangoConfiguration {

    @Bean
    @Qualifier("arangoBuilderForIds")
    public ArangoDB.Builder produceIdsArangoDB(
    ...
    @Bean
    @Qualifier("idsDB")
    public ArangoDatabaseProxy produceIdsDB(@Qualifier("arangoBuilderForIds") ArangoDB.Builder arangoDB) {
        return new ArangoDatabaseProxy(arangoDB.build(), "kg1-ids");
    }
